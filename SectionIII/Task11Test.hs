module SectionIII.Task11Test
  ( sectionIIITask11Tests
  , main
  ) where

import Data.List (isPrefixOf)
import SectionIII.Task11Solution
  ( Automaton(..)
  , Word
  , Problem(..)
  , Transition(..)
...
import TestRunner (Assertion, Test, assertEqual, runTestMain, testCase, testGroup)
import Prelude hiding (Word)

main :: IO ()
main = runTestMain sectionIIITask11Tests
...
sectionIIITask11Tests =
  testGroup
    "Section III, task 11"
    [ testCase
        "v=a, w=b, automaton accepts acbc -> expected valid word vywy"
        (assertPositive nonEmptyYProblem)
    , testCase
        "v=a, w=b, automaton accepts ab -> expected valid word with empty y"
        (assertPositive emptyYProblem)
    , testCase
        "v=a, w=b, automaton cannot place b -> expected Nothing"
        (assertEqual Nothing (solve negativeProblem))
    , testCase
        "v=a, w=b, nondeterministic automaton -> expected valid word vywy"
        (assertPositive nondeterministicProblem)
    , testCase
        "v=p, w=q, partial transition automaton -> expected valid word vywy"
        (assertPositive missingTransitionsProblem)
    , testCase
        "v=a, w=b, multiple accepting states -> expected valid word vywy"
