...
У нескорочуваній граматиці для кожного нетермінала A знайти minlengthL(A) = min{length(w) | A =>+ wA, w - слово в термінальному алфавіті} та words_minlenL(A) = {w | A =>+ wA, w - слово в термінальному алфавіті, length(w) = minlengthL(A)}.

## Код програми

```haskell
module SectionIV.Task7Solution
  ( Nonterminal
  , Terminal
  , TerminalWord
  , Symbol(..)
...
1. Пряма ліва рекурсія `A -> aA` перевіряє базовий випадок, коли мінімальна довжина слова перед повторним `A` дорівнює 1.
2. Та сама пряма рекурсія перевіряє, що множина мінімальних слів містить саме слово `"a"`.
3. Непряма рекурсія через інший нетермінал перевіряє побудову ланцюжка `A -> bB -> bcA` і знаходження слова `"bc"`.
4. Правило `A -> A` перевіряє випадок порожнього слова перед повторним нетерміналом, тобто мінімальну довжину 0.
5. Граматика з двома однаково короткими варіантами перевіряє, що алгоритм повертає всі мінімальні слова, а не тільки перше знайдене.
6. Граматика без повернення до початкового нетермінала перевіряє негативний випадок, коли значення `minlengthL(A)` не існує.
7. Граматика, де префікс перед `A` породжується іншим нетерміналом, перевіряє врахування мінімальних термінальних слів для допоміжних нетерміналів.
8. Правило з нетерміналом не в кінці правої частини перевіряє, що такі правила не вважаються коректним виглядом `wA` для лівої рекурсії.
9. Перевірка `solve` для кількох нетерміналів показує, що алгоритм повертає результат для кожного нетермінала граматики, включно з випадком відсутності рекурсії.

## Екранний знімок з результатами виконання тестів

![Результати тестів задачі 7](../assets/section-iv-task-7.hs.png)
